...
/area
	...
	var/valid_territory = FALSE // Is it a valid territory for gangs to claim? See misc_areas.dm
	var/blob_allowed = FALSE // Does it count for blobs score? See misc_areas.dm
...
/area/proc/add_delta_areas()
	if(is_station_level(z) && !istype(src, /area/external/shuttle) && !istype(src, /area/ruins) && !istype(src, /area/space))
