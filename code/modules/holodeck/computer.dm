...
/obj/machinery/computer/holodeck
	...
	var/area/mapped_start_area = /area/debug/holodeck/main
	...
	var/area/debug/holodeck/linked
...
/obj/machinery/computer/holodeck/LateInitialize()//from here linked is populated and the program list is generated. its also set to load the offline program
	...
	if(istype(computer_area, /area/debug/holodeck))
...
/obj/machinery/computer/holodeck/proc/generate_program_list()
	for(var/datum/map_template/holodeck/program as anything in subtypesof(program_type))
		if(program in blacklisted_programs)
			continue
...
/obj/machinery/computer/holodeck/ui_act(action, params)
	...
	switch(action)
		...
		if("safety")
			if (!(obj_flags & EMAGGED) && !(issilicon(usr) || IsAdminGhost(usr)))
				return
...
/obj/machinery/computer/holodeck/proc/derez(atom/movable/holo_atom, silent = TRUE, forced = FALSE)
	...
	if(istype(holo_atom, /obj/item/clothing/under))
		...
		holo_clothing.remove_accessory(forced = TRUE)
	if(!silent)
...
/obj/machinery/computer/holodeck/process(seconds_per_tick)
	if(damaged && DT_PROB(5, seconds_per_tick))
...
/obj/machinery/computer/holodeck/power_change()
	. = ..()
	INVOKE_ASYNC(src, PROC_REF(toggle_power), !stat)
