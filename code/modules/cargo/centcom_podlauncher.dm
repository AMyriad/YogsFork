...
/datum/centcom_podlauncher
	...
	var/area/debug/admin/centcom/supplypod/loading/bay //What bay we're using to launch shit from.
...
/datum/centcom_podlauncher/proc/setup(user) //H can either be a client or a mob
	...
	bay =  locate(/area/debug/admin/centcom/supplypod/loading/one) in GLOB.areas //Locate the default bay (one) from the centcom map
	bayNumber = bay.loading_id //Used as quick reference to what bay we're taking items from
	var/area/pod_storage_area = locate(/area/debug/admin/centcom/supplypod/storage) in GLOB.areas
...
/datum/centcom_podlauncher/ui_act(action, params)
	...
	switch(action)
		...
		if("clearBay") //Delete all mobs and objs in the selected bay
			if(tgui_alert(usr, "This will delete all objs and mobs in [bay]. Are you sure?", "Confirmation", "Delete that shit", "No") == "Delete that shit")
				clearBay()
				refreshBay()
			. = TRUE
...
/datum/centcom_podlauncher/proc/createOrderedArea(area/area_to_order) //This assumes the area passed in is a continuous square
	if (isnull(area_to_order)) //If theres no supplypod bay mapped into centcom, throw an error
		to_chat(holder.mob, "No /area/debug/admin/centcom/supplypod/loading/one (or /two or /three or /four) in the world! You can make one yourself (then refresh) for now, but yell at a mapper to fix this, today!")
		CRASH("No /area/debug/admin/centcom/supplypod/loading/one (or /two or /three or /four) has been mapped into the centcom z-level!")
	orderedArea = list()
...
/datum/centcom_podlauncher/proc/launch(turf/target_turf) //Game time started
	...
	var/shippingLane = GLOB.areas_by_type[/area/debug/admin/centcom/supplypod/storage]
